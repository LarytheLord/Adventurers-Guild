...
export default function HomePageClient({ quests }) {
  ...
  const filteredQuests = quests.filter(quest => {
    ...
    const matchesSearch = quest.title.toLowerCase().includes(searchTermLower) ||
                          quest.description.toLowerCase().includes(searchTermLower);
    const matchesRank = rankFilter === 'all' || quest.difficulty === rankFilter;
      ...
      <main>
        ...
        <section id="quests" className="px-6 py-8 max-w-6xl mx-auto">
          ...
          <Suspense fallback={<p>Loading quests...</p>}>
            <QuestBoard quests={filteredQuests} />
          </Suspense>
        </section>
      </main>
