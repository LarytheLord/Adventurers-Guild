...
type RazorpayConstructor = new (config: { key_id: string; key_secret: string }) => Razorpay;

function loadRazorpay(): RazorpayConstructor {
  try {
    return eval('require')('razorpay') as RazorpayConstructor;
  } catch {
    throw new Error('The Razorpay SDK is not installed in this workspace');
  }
}
...
export function getRazorpay(): Razorpay {
  if (!razorpayInstance) {
    ...
    const RazorpayClient = loadRazorpay();
    razorpayInstance = new RazorpayClient({ key_id: keyId, key_secret: keySecret });
  }
